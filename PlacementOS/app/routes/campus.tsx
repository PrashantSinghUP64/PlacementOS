...
export default function CampusTracker() {
  ...
  const generateInsights = async () => {
    if (!(window as any).puter?.ai?.chat || records.length === 0) return;
...
Return ONLY JSON:
{
  ...
  "recommendations": ["string", "string", "string"]
    ...
    try {
      ...
      const jsonStr = (res?.message?.content || "").replace(/```json\n?|\n?```/g, "").trim();
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans">
        ...
        <div className="max-w-7xl mx-auto relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div>
            <h1 className="text-4xl md:text-5xl font-black mb-2 flex items-center gap-3">
              <span className="bg-white/10 p-2 rounded-xl text-3xl">🏫</span>
          ...
          <button onClick={() => openModal()} className="bg-white text-fuchsia-900 hover:bg-fuchsia-50 font-bold py-3 px-8 rounded-xl shadow-lg transition-transform hover:scale-105 whitespace-nowrap">
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-16 relative z-20">
        ...
        {/* Filters Card */}
        <div className="bg-white rounded-3xl shadow-xl flex flex-col md:flex-row gap-4 p-4 border border-gray-100 items-center justify-between mb-8">
          <div className="flex-1 w-full">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-3 bg-gray-50 rounded-2xl outline-none focus:ring-2 focus:ring-fuchsia-500 font-bold text-gray-800 transition-all border-transparent"
              />
            </div>
          </div>
          <div className="flex gap-4 w-full md:w-auto">
            <select 
              ...
              className="py-3 px-4 bg-gray-50 rounded-2xl font-bold text-gray-700 outline-none focus:ring-2 focus:ring-fuchsia-500 min-w-[120px]"
            >
            ...
            <select 
              ...
              className="py-3 px-4 bg-gray-50 rounded-2xl font-bold text-gray-700 outline-none focus:ring-2 focus:ring-fuchsia-500"
            >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex flex-col items-center justify-center text-center">
            ...
            <p className="text-3xl font-black text-gray-900">{avgPackage} <span className="text-lg">LPA</span></p>
          </div>
        </div>
        ...
        <div className="flex flex-col xl:flex-row gap-8">
          ...
          <div className="flex-1 bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-bold text-gray-900">Placement Records ({filteredRecords.length})</h2>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">Role</th>
                      <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">Package</th>
                      <th className="p-4 text-xs font-bold text-gray-500 uppercase tracking-wider border-b border-gray-200">Students</th>
                  ...
                  <tbody>
                    {filteredRecords.map((r, i) => (
                      <tr key={r._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                        ...
                        <td className="p-4 font-medium text-gray-800">{r.role}</td>
                        <td className="p-4">
                          <span className="bg-green-100 text-green-700 px-3 py-1 rounded-lg font-bold text-sm">
                            {r.package} LPA
                          </span>
                        </td>
                        <td className="p-4 font-bold text-gray-700">{r.studentsSelected}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4 text-right">
                          {r.userId === user?.id && (
                            <div className="flex gap-2 justify-end">
                              <button onClick={() => openModal(r)} className="text-gray-400 hover:text-blue-600 bg-white shadow-sm border py-1 px-3 rounded-lg text-sm font-medium">Edit</button>
                              <button onClick={() => handleDelete(r._id)} className="text-red-400 hover:text-red-600 bg-white shadow-sm border py-1 px-3 rounded-lg text-sm font-medium">Delete</button>
                            </div>
                          )}
                        </td>
                      </tr>
            ...
            <div className="bg-gradient-to-br from-indigo-900 to-purple-900 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden h-max">
              ...
              {!insights ? (
                <button 
                  ...
                  className="w-full bg-white text-indigo-900 hover:bg-indigo-50 font-bold py-3 rounded-xl shadow-lg transition-transform hover:-translate-y-1 disabled:opacity-50 flex items-center justify-center gap-2"
                >
                ...
                <div className="space-y-6 animate-fade-in">
                  <div>
                    ...
                    <p className="bg-white/10 p-3 rounded-xl text-sm leading-relaxed">{insights.summary}</p>
                  </div>
                  <div>
                    <h4 className="text-indigo-200 font-bold text-xs uppercase tracking-wider mb-2">Trend</h4>
                    <p className="bg-white/10 p-3 rounded-xl text-sm leading-relaxed">{insights.trend}</p>
                  </div>
                  <div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-fade-in-up">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-bold text-gray-900">{editingRecord ? 'Edit Placement Record' : 'Add Placement Record'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 bg-white shadow-sm border border-gray-100 w-8 h-8 rounded-full flex items-center justify-center font-bold">✕</button>
            </div>
            ...
            <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Students Selected *</label>
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-1">Interview Process</label>
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-1">Notes / Tips</label>
              ...
              <div className="pt-4 border-t border-gray-100 mt-6 flex justify-end gap-3 z-50">
