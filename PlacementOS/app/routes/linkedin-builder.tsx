      ...
      <div className="max-w-5xl mx-auto px-6 -mt-14 relative z-10">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-black text-gray-900">Your Progress</h2>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          {step === 1 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6 flex items-center gap-2">📸 Profile Photo Tips</h2>
          ...
          {step === 2 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">✍️ Headline Generator</h2>
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                <div><label className="block text-sm font-bold text-gray-700 mb-1.5">Your Name</label><input value={name} onChange={e => setName(e.target.value)} placeholder="Prashant Kumar Singh" className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500" /></div>
                <div><label className="block text-sm font-bold text-gray-700 mb-1.5">Target Role</label><input value={targetRole} onChange={e => setTargetRole(e.target.value)} placeholder="Full Stack Developer" className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500" /></div>
                <div className="md:col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1.5">Top Skills (comma separated)</label><input value={skills} onChange={e => setSkills(e.target.value)} placeholder="React.js, Node.js, Python, DSA" className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500" /></div>
              </div>
              ...
              {generated && (
                <div className="mt-4 relative">
                  <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl whitespace-pre-wrap text-sm font-medium text-gray-800">{generated}</div>
          ...
          {step === 3 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">📝 About / Summary Writer</h2>
              <div className="grid md:grid-cols-2 gap-4 mb-4">
                {[["Name", name, setName, "Prashant Kumar"], ["College", college, setCollege, "AKTU, Lucknow"], ["Target Role", targetRole, setTargetRole, "SDE / Full Stack"], ["Skills", skills, setSkills, "React, Node, Python"]].map(([label, val, setter, ph]) => (
                  <div key={label as string}><label className="block text-sm font-bold text-gray-700 mb-1.5">{label as string}</label><input value={val as string} onChange={e => (setter as (v: string) => void)(e.target.value)} placeholder={ph as string} className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500" /></div>
                ))}
                <div className="md:col-span-2"><label className="block text-sm font-bold text-gray-700 mb-1.5">Key Projects (1-2 lines each)</label><textarea value={projects} onChange={e => setProjects(e.target.value)} rows={3} placeholder="E-commerce app with React + Node. Resume analyzer with AI." className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500 resize-none" /></div>
              </div>
              ...
              {generated && (
                <div className="mt-4 relative">
                  <div className="p-4 bg-gray-50 border border-gray-200 rounded-xl whitespace-pre-wrap text-sm font-medium text-gray-800 max-h-60 overflow-y-auto">{generated}</div>
          ...
          {step === 4 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">💼 Experience Section</h2>
              <div className="mb-4"><label className="block text-sm font-bold text-gray-700 mb-1.5">Describe your internship / job experience</label><textarea value={experience} onChange={e => setExperience(e.target.value)} rows={4} placeholder="Company: Tech Mahindra, Role: SDE Intern, Duration: 2 months. I worked on REST APIs using Node.js, fixed bugs in the payment module, wrote unit tests." className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500 resize-none" /></div>
          ...
          {step === 5 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">🛠️ Skills Section</h2>
              <div className="mb-4">
                <label className="block text-sm font-bold text-gray-700 mb-2">Target Role</label>
                <div className="flex gap-2 flex-wrap">
                  {Object.keys(SKILL_SUGGESTIONS).map(r => (
                    <button key={r} onClick={() => setRole(r)} className={`px-4 py-2 rounded-xl font-bold text-sm border-2 transition-all ${role === r ? "bg-[#0A66C2] text-white border-[#0A66C2]" : "border-gray-200 text-gray-600 hover:border-blue-300"}`}>{r}</button>
                  ))}
                </div>
              </div>
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {SKILL_SUGGESTIONS[role].map(s => (
                    <span key={s} className="px-3 py-1.5 bg-white border border-blue-200 text-blue-800 rounded-full text-sm font-bold">{s}</span>
                  ))}
                </div>
              </div>
          ...
          {step === 6 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">🚀 Projects Section</h2>
              <div className="mb-4"><label className="block text-sm font-bold text-gray-700 mb-1.5">Describe your project</label><textarea value={projects} onChange={e => setProjects(e.target.value)} rows={3} placeholder="Project: Resume Analyzer. Tech: React, Node, MongoDB, OpenAI. What it does: Analyzes resumes and gives ATS score. Link: github.com/prashant/resume-ai" className="w-full p-3 border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500 resize-none" /></div>
          ...
          {step === 7 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">🎓 Education Section</h2>
              <div className="space-y-3">
                {[["What to include", ["Degree, branch, years (e.g. B.Tech CSE 2021-2025)", "CGPA — include it (if above 7.0)", "Relevant coursework: DSA, DBMS, OS, CN, ML", "Achievements: Hackathon wins, Dean's list, scholarships", "Activities: Tech clubs, IEEE, coding club lead"]], ["What to skip", ["School (10th/12th) — optional if below 65%", "Irrelevant activities", "CGPA if below 6.5 (just put 'B.Tech CSE')"]]].map(([title, items]) => (
                  <div key={title as string} className={`p-4 rounded-xl border ${title === "What to include" ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}>
          ...
          {step === 8 && (
            <div>
              <h2 className="text-2xl font-black text-gray-900 mb-6">🤝 Connection Strategy</h2>
              <p className="text-gray-600 font-medium mb-5 text-sm">First 500 connections are most important. Target: seniors from your college, recruiters in target companies, professionals in your domain.</p>
                ...
                {MSG_TEMPLATES.map((t, i) => (
                  <div key={i} className="bg-gray-50 border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    <div className="p-4 relative">
                      <pre className="text-xs font-medium text-gray-700 whitespace-pre-wrap">{t.msg}</pre>
        ...
        <div className="flex justify-between">
          <button onClick={() => setStep(s => Math.max(1, s - 1))} disabled={step === 1} className="px-6 py-3 bg-gray-100 hover:bg-gray-200 disabled:opacity-40 text-gray-700 font-bold rounded-xl transition-colors">← Previous</button>
          {step < 8 ? (
