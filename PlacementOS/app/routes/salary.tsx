...
export default function SalaryPredictor() {
  ...
  const handlePredict = async () => {
    ...
    try {
      if (!window.puter?.ai?.chat) throw new Error("Puter.js AI is not ready. Please wait a moment.");
    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-16 relative z-20">
        <div className="grid lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8 animate-fade-in-up">
            <h2 className="text-lg font-bold text-gray-900 mb-6 border-b border-gray-100 pb-4">Your Profile</h2>
            
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Job Role</label>
                <select value={role} onChange={e => setRole(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-yellow-500 transition-all text-sm font-medium">
              ...
              <div>
                ...
                <select value={location} onChange={e => setLocation(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-yellow-500 transition-all text-sm font-medium">
              ...
              <div>
                ...
                <select value={education} onChange={e => setEducation(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-yellow-500 transition-all text-sm font-medium">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-yellow-500 transition-all text-sm mb-3"
                />
            ...
            {error && (
              <div className="mt-6 p-3 bg-red-50 text-red-600 rounded-lg border border-red-100 flex items-center gap-2 text-sm font-medium">
          ...
          <div className="lg:col-span-8">
            {result ? (
              <div className="space-y-6 animate-fade-in-up delay-100">
                ...
                {/* Top Insight Card */}
                <div className="bg-white rounded-2xl shadow-sm border border-yellow-200 p-5 flex gap-4 items-start relative overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <h3 className="font-bold text-gray-900 mb-1">Market Insight</h3>
                    <p className="text-sm text-gray-700 leading-relaxed font-medium">{result.insight}</p>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 md:p-10 text-center relative overflow-hidden">
                  ...
                  <p className="text-lg text-gray-500 font-medium">
                    Average: <span className="text-gray-900 font-black">{formatCurrency(result.avgSalary, result.currency)}</span>
                  </p>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[
                    { title: "Entry Level", value: result.entryLevel, sub: "0-2 Years", pattern: "bg-blue-50/50" },
                    { title: "Mid Level", value: result.midLevel, sub: "3-5 Years", pattern: "bg-violet-50/50" },
                    { title: "Senior Level", value: result.seniorLevel, sub: "6+ Years", pattern: "bg-emerald-50/50" }
                  ].map((lvl, i) => (
                    <div key={i} className={`bg-white rounded-2xl shadow-sm border border-gray-100 p-5 ${lvl.pattern}`}>
                      <h3 className="text-sm font-bold text-gray-500 mb-1">{lvl.title}</h3>
                      <p className="text-xl md:text-2xl font-black text-gray-900 mb-1">{formatCurrency(lvl.value, result.currency)}</p>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="font-bold text-gray-900 text-lg">Top Paying Companies</h3>
                    </div>
                    <div className="space-y-4">
                      {result.topCompanies.map((c, i) => (
                        <div key={i} className="flex items-center justify-between group">
                          ...
                          <span className="font-black text-gray-900">{formatCurrency(c.avgSalary, result.currency)}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8">
                    <div className="flex items-center gap-3 mb-6">
                      ...
                      <h3 className="font-bold text-gray-900 text-lg">Negotiation Strategy</h3>
                    </div>
                    <ul className="space-y-5 relative before:absolute before:inset-y-0 before:left-3 before:-ml-px before:w-0.5 before:bg-gray-100">
                      {result.negotiationTips.map((tip, i) => (
                        <li key={i} className="relative flex gap-4">
                          <div className="flex-none w-6 h-6 rounded-full bg-white border-2 border-purple-500 flex items-center justify-center -ml-0.5 z-10 shadow-sm text-[10px] font-black text-purple-600">
                            {i+1}
                          </div>
              ...
              <div className="h-[600px] flex flex-col items-center justify-center text-center p-8 bg-gray-50/50 rounded-2xl border-2 border-dashed border-gray-200">
                <div className="w-20 h-20 bg-white rounded-full shadow-sm border border-gray-100 flex items-center justify-center text-4xl mb-4 relative">
                ...
                <h3 className="text-xl font-bold text-gray-900 mb-2">Awaiting Your Data</h3>
