  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans">
      ...
      <div className="max-w-5xl mx-auto px-6 -mt-8">
        ...
        {!results && (
          <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 md:p-12 max-w-3xl mx-auto animate-fade-in-up">
            <h2 className="text-2xl font-bold text-gray-900 mb-8 border-b pb-4">Define Your Target</h2>
            
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Current Skills (Press Enter to add)</label>
                <div className="flex flex-wrap gap-2 mb-3 bg-gray-50 p-3 rounded-xl min-h-[50px] border border-gray-200 focus-within:border-teal-400 focus-within:ring-4 focus-within:ring-teal-50 transition-all">
                  ...
                  <input
                    ...
                    className="flex-1 min-w-[150px] bg-transparent outline-none text-gray-800 placeholder-gray-400"
                  />
                </div>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Dream Job Title <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                    value={dreamJob}
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">Dream Company (Optional)</label>
                  <input
                    type="text"
                    value={dreamCompany}
        ...
        {results && (
          <div className="animate-fade-in-up space-y-8">
            {/* Top Score */}
            <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-8 flex flex-col items-center justify-center text-center relative overflow-hidden">
              ...
              <div className="relative w-48 h-48 mb-6 flex items-center justify-center">
                ...
                <div className="absolute flex flex-col items-center justify-center">
                  <span className="text-5xl font-black text-gray-800">{results.readinessScore}%</span>
                  <span className="text-xs font-bold text-gray-400 mt-1 uppercase">Ready</span>
                </div>
              </div>

              <h2 className="text-2xl font-bold text-gray-800 max-w-xl">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {/* Column 1: Strengths */}
              <div className="bg-white border-t-4 border-green-500 rounded-2xl p-6 shadow-sm flex flex-col">
                ...
                <ul className="space-y-3 flex-1 overflow-y-auto max-h-[400px]">
                  {results.matchingSkills.map((skill: string, i: number) => (
                    <li key={i} className="flex items-center gap-3 bg-green-50/50 p-3 rounded-xl border border-green-100">
                      <span className="text-green-500 bg-white shadow-sm w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 font-bold text-xs">✓</span>
              ...
              <div className="bg-white border-t-4 border-red-500 rounded-2xl p-6 shadow-sm flex flex-col">
                <h3 className="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2">
                ...
                <div className="space-y-3 flex-1 overflow-y-auto max-h-[400px]">
                  {results.missingSkills.map((item: any, i: number) => (
                    <div 
                      ...
                      className={`cursor-pointer transition-all border p-3 rounded-xl ${expandedSkill === item.skill ? 'bg-red-50 border-red-300' : 'bg-red-50/30 border-red-100 hover:border-red-300'}`}
                    >
                      <div className="flex justify-between items-start mb-1">
                        <span className="font-bold text-gray-800">{item.skill}</span>
                        <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${item.priority === 'Must Have' ? 'bg-red-200 text-red-800' : item.priority === 'Good to Have' ? 'bg-yellow-200 text-yellow-800' : 'bg-gray-200 text-gray-800'}`}>
                          {item.priority}
                        </span>
                      </div>
              ...
              <div className="bg-white border-t-4 border-yellow-400 rounded-2xl p-6 shadow-sm flex flex-col">
                <h3 className="text-lg font-bold text-gray-900 mb-2 flex items-center gap-2">
                ...
                <div className="space-y-3 flex-1 overflow-y-auto max-h-[400px]">
                  {results.skillsToImprove.map((item: any, i: number) => (
                    <div key={i} className="bg-yellow-50/50 border border-yellow-100 p-3 rounded-xl">
                      <h4 className="font-bold text-gray-800 mb-1">{item.skill}</h4>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
              ...
              <div className="grid md:grid-cols-4 gap-6">
                {['week1', 'week2', 'week3', 'week4'].map((week, idx) => {
                  ...
                  return (
                    <div key={week} className="bg-gray-50 rounded-2xl p-5 border border-gray-200 shadow-inner">
                      <div className="flex items-center justify-between mb-4 pb-4 border-b border-gray-200">
                        <span className="font-black text-gray-900 uppercase">Week {idx + 1}</span>
                      ...
                      <ul className="space-y-4">
                        {items.map((item: any, i: number) => (
                          <li key={i} className="text-sm">
                            <p className="font-bold text-gray-800 leading-tight mb-1">{item.topic}</p>
                ...
                <div className="grid md:grid-cols-2 gap-8 relative z-10">
                  <div className="space-y-6">
                    <div>
                      ...
                      <span className={`inline-block px-3 py-1 rounded-full text-sm font-black ${results.companyInsights.difficulty === 'High' ? 'bg-red-500/20 text-red-300 border border-red-500/50' : 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/50'}`}>
                    ...
                    <div>
                      ...
                      <p className="text-blue-50 font-medium leading-relaxed bg-white/5 p-4 rounded-xl border border-white/10">
            ...
            <div className="text-center pt-8">
              <button onClick={() => setResults(null)} className="text-gray-500 font-medium hover:text-gray-800 underline">
