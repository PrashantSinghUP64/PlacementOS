    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 pb-20 font-sans">
      ...
      <div className="bg-gradient-to-br from-amber-500 to-orange-600 text-white pt-16 pb-28">
        <div className="max-w-4xl mx-auto px-6 text-center">
          ...
          <div className="mt-4 p-3 bg-white dark:bg-gray-900/20 rounded-xl inline-block">
      ...
      <div className="max-w-6xl mx-auto px-6 -mt-14 relative z-10 space-y-8">
          ...
          <div className="flex flex-wrap gap-2">
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${tab === t ? "bg-amber-50 dark:bg-amber-900/200 text-white" : "bg-gray-100 text-gray-600 dark:text-gray-400 hover:bg-gray-200"}`}>{t}</button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {CERTS[tab]?.map((cert, i) => (
            <div key={i} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
              <div className="flex items-start gap-4 p-5">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <h3 className="font-black text-gray-900 dark:text-white">{cert.name}</h3>
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${priorityColors[cert.priority] || "bg-gray-100 text-gray-700 dark:text-gray-300"}`}>{cert.priority}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">{cert.free.map((f, j) => <span key={j} className="text-xs font-medium px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-700 rounded-lg">{f}</span>)}</div>
                  </div>
                </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
          <h2 className="text-xl font-black text-gray-900 dark:text-white mb-4">AI Personalized Cert Path 🤖</h2>
          <div className="grid md:grid-cols-2 gap-3 mb-4">
            {[["Target Role", "role", "Full Stack Dev / Data Scientist"], ["Current Skills", "skills", "React, Python, basic AWS"], ["Budget", "budget", "₹5000 or 'limited'"], ["Timeline", "time", "3 months / 6 months"]].map(([l, k, p]) => (
              <div key={k}><label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">{l}</label><input value={aiIn[k as keyof typeof aiIn]} onChange={e => setAiIn(a => ({ ...a, [k]: e.target.value }))} placeholder={p} className="w-full p-2.5 border border-gray-200 dark:border-gray-800 rounded-xl text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none" /></div>
            ))}
          </div>
          <button onClick={getAiRec} disabled={loading || !aiIn.role.trim()} className="px-6 py-3 bg-amber-50 dark:bg-amber-900/200 hover:bg-amber-600 disabled:bg-gray-200 text-white font-black rounded-xl transition-colors">
            {loading ? "Getting recommendations..." : "Get My Cert Path 🏅"}
          </button>
          {aiOut && <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 rounded-xl text-sm font-medium text-gray-800 dark:text-gray-200 whitespace-pre-wrap">{aiOut}</div>}
        </div>

        {/* Tracker */}
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 p-6">
          <h2 className="text-xl font-black text-gray-900 dark:text-white mb-4">Certification Tracker 📋</h2>
          <div className="flex gap-3 mb-4">
            <input value={newCert.name} onChange={e => setNewCert(c => ({ ...c, name: e.target.value }))} placeholder="Certificate name" className="flex-1 p-2.5 border border-gray-200 dark:border-gray-800 rounded-xl text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none" />
            <select value={newCert.status} onChange={e => setNewCert(c => ({ ...c, status: e.target.value }))} className="p-2.5 border border-gray-200 dark:border-gray-800 rounded-xl text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none">
            ...
            <button onClick={() => { if (newCert.name) { setTracker(t => [...t, { name: newCert.name, status: newCert.status, date: new Date().toLocaleDateString("en-IN") }]); setNewCert({ name: "", status: "In Progress" }); } }}
              className="px-4 py-2.5 bg-amber-50 dark:bg-amber-900/200 text-white font-bold rounded-xl text-sm hover:bg-amber-600 transition-colors">Add</button>
          </div>
          {tracker.length === 0 ? <p className="text-gray-400 text-sm text-center py-4">Add certifications you're working on</p> : (
            <div className="space-y-2">
              {tracker.map((c, i) => (
                <div key={i} className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-950 border border-gray-200 dark:border-gray-800 rounded-xl">
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${c.status === "Completed" ? "bg-green-100 text-green-700" : c.status === "In Progress" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-600 dark:text-gray-400"}`}>{c.status}</span>
                  <span className="flex-1 font-medium text-gray-800 dark:text-gray-200 text-sm">{c.name}</span>
