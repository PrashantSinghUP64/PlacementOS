...
export default function JobTracker() {
  ...
  const columns: { title: string; status: JobStatus; color: string; bg: string; borderColor: string }[] = [
    { title: "Applied", status: "Applied", color: "text-blue-700", bg: "bg-blue-50/50", borderColor: "border-blue-200" },
    { title: "Interview", status: "Interview", color: "text-yellow-700", bg: "bg-yellow-50/50", borderColor: "border-yellow-200" },
    { title: "Offered", status: "Offered", color: "text-green-700", bg: "bg-green-50/50", borderColor: "border-green-200" },
    { title: "Rejected", status: "Rejected", color: "text-red-700", bg: "bg-red-50/50", borderColor: "border-red-200" }
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 font-sans">
      ...
      {/* HEADER SECTION */}
      <div className="bg-white border-b border-gray-200 pt-8 pb-6 px-6 shadow-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="flex gap-4 items-center">
              <div className="relative">
                ...
                <input
                  ...
                  className="pl-9 pr-4 py-2 bg-gray-100 border-transparent rounded-xl focus:bg-white focus:ring-2 focus:ring-blue-500 text-sm font-medium w-full md:w-64 transition-all outline-none"
                />
              </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            ...
            <div className="bg-blue-50 rounded-2xl p-4 border border-blue-100 flex flex-col justify-end">
              ...
              {columns.map(col => (
                ...
                >
                  <div className={`px-4 py-3 rounded-t-2xl font-black border-b ${col.bg} ${col.color} ${col.borderColor} flex justify-between items-center`}>
                    <span>{col.title}</span>
                    <span className="bg-white/50 px-2 py-0.5 rounded-lg text-sm">{filteredJobs.filter(j => j.status === col.status).length}</span>
                  </div>
                  
                  <div className={`flex-1 p-3 bg-white border border-t-0 border-gray-200 rounded-b-2xl shadow-sm space-y-3 ${draggedJobId ? 'bg-gray-50/50' : ''}`}>
                    {filteredJobs.filter(j => j.status === col.status).map(job => (
                      <div
                        ...
                        className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm hover:shadow hover:border-blue-300 transition-all cursor-grab active:cursor-grabbing group relative"
                      >
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex bg-white/90 rounded shadow-sm border border-gray-100 backdrop-blur-sm z-10">
                        ...
                        <h3 className="font-bold text-gray-900 pr-12">{job.company}</h3>
                        <p className="text-sm font-medium text-gray-600 mb-3">{job.jobTitle}</p>
                        
                        {job.salary && (
                          <div className="inline-block bg-green-50 text-green-700 text-xs font-bold px-2 py-1 rounded mb-3 border border-green-100">
                            💰 {job.salary}
                          </div>
                        )}

                        <div className="flex justify-between items-center text-xs font-semibold text-gray-400 pt-3 border-t border-gray-100 mt-2">
                    ...
                    {filteredJobs.filter(j => j.status === col.status).length === 0 && (
                      <div className="h-24 border-2 border-dashed border-gray-200 rounded-xl flex items-center justify-center text-sm font-medium text-gray-400 opacity-50">
                        Drop here
                      </div>
                    )}
                  </div>
            ...
            {jobs.length > 0 && (
              <div className="grid md:grid-cols-2 gap-6 mt-8">
                <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
                  <h3 className="text-lg font-bold text-gray-800 mb-6">Applications Trend (Last 4 Weeks)</h3>
                  <div className="h-64">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-fade-in-up">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
              <h2 className="text-xl font-bold text-gray-900">{editingJob ? 'Edit Application' : 'Add Application'}</h2>
            ...
            <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">Applied Date</label>
                ...
                <div className="col-span-2">
                  <label className="block text-sm font-bold text-gray-700 mb-1">Job URL</label>
